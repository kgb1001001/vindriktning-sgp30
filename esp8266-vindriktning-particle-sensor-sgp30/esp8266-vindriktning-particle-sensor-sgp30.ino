...
void publishState() {
    ...
    DynamicJsonDocument stateJson(644);
    char payload[256];
    ...
    uint16_t result = 5;
    uint16_t coresult = 5 - ((eco2-400)/100);
    uint16_t pmresult = 5 - (state.avgPM25/10);
    uint16_t vocresult = 5 - (tvoc/50);

    result = (coresult < pmresult) ? coresult : pmresult;
    result = (result < vocresult) ? result : vocresult;
